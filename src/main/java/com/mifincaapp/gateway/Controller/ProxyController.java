...
public class ProxyController {
    ...
    // --------------------- MÉTODOS GENERALES ---------------------
    private ResponseEntity<?> proxyRequest(HttpServletRequest request, String baseUrl, boolean requiereToken) {
        return proxyRequest(request, baseUrl, requiereToken, true);
    }

    private ResponseEntity<?> proxyRequest(HttpServletRequest request, String baseUrl, boolean requiereToken, boolean requiereHeaderCliente) {
        try {
            ...
            if (requiereHeaderCliente && !headers.containsKey("USER-MIFINCA-CLIENT")) {
                return ResponseEntity.badRequest().body("Falta el header requerido: USER-MIFINCA-CLIENT");
            }

            if (requiereToken && !headers.containsKey("Authorization")) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Falta el token en el header Authorization");
            }

            HttpMethod method = HttpMethod.valueOf(request.getMethod());

            HttpEntity<?> entity = (method == HttpMethod.DELETE)
                    ? new HttpEntity<>(headers)
                    : new HttpEntity<>(bodyBytes.length > 0 ? bodyBytes : null, headers);
